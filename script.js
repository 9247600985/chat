...
const DOM =  {
	...
	preprofilePic: getById("pre-profile-pic"),
	preprofilePicInput: getById("pre-profile-pic-input"),
	inputName: getById("input-name"),
	username: getById("username"),
	displayPic: getById("display-pic"),
	btnRegister:getById('register')
};
...
let preinit = () => {
	...
	DOM.preprofilePic.addEventListener("click", () => DOM.preprofilePicInput.click());
	DOM.preprofilePicInput.addEventListener("change", () => {
		console.log(DOM.preprofilePicInput.files[0]);
		var reader = new FileReader();
        reader.onloadend = function() {
             DOM.preprofilePic.src = reader.result;
        }
        reader.readAsDataURL(DOM.preprofilePicInput.files[0]);
	});
};
$('#postLogin').hide();
preinit();
...
function myFunction(event){
	var elem = $("#sender");
	if(elem.hasClass('fa-paper-plane') && event.value.length == 0){
		elem.removeClass('fa-paper-plane');
		elem.addClass('fa-microphone');
		return;
	}
	if(elem.hasClass('fa-microphone')){
		elem.removeClass("fa-microphone");
